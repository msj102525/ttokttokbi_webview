 ...
 */
@Controller(value = "ttbContentsController")
public class TtbContentsController {

	private static final Logger log = LoggerFactory.getLogger(TtbContentsController.class);

	@Resource(name = "ttbTemplateService")
	private ITemplateService templateService;

	@RequestMapping(value = "/ttb/contents/images/url", method = { RequestMethod.GET, RequestMethod.POST })
		...
		if (!"".equals(id) && !"".equals(s_id) && !"".equals(c_id)) {
			...
			try {
				...
				logData.append("[" + LogUtils.getCurrentTime() + "]" + " "
						+ "query start selectContents TemplateVo templateVo" + templateVo + "\n");
				templateVo = templateService.selectContents(templateVo);
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
				logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "query error selectContents : "
						+ e.toString() + "\n");
			}
			logData.append("[" + LogUtils.getCurrentTime() + "]" + " "
					+ "query end selectContents TemplateVo templateVo : " + templateVo + "\n");
			if (templateVo != null) {
