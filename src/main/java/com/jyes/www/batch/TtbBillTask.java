...
import com.jyes.www.service.ttb.custom.ICustomService;
...
public class TtbBillTask {
	...
	public void execute() {
		...
		try {
			...
			e.printStackTrace();
			logData.append(
					"[" + LogUtils.getCurrentTime() + "]" + " " + "query error sendMailForm : " + e.toString() + "\n");
		}
		logData.append(
				"[" + LogUtils.getCurrentTime() + "]" + " " + "query end sendMailForm int success : " + success + "\n");
	...
	@Resource(name = "ttbCustomService")
	...
	public void taskScheduler() {
		log.info("taskScheduler 개발서버:10.255.0.15, 상용서버:10.131.101.13 NetUtils.getServerIp() : "
				+ NetUtils.getServerIp());
		if ("10.255.0.15".equals(StringUtil.nvl(NetUtils.getServerIp()))
				...
				if (al_PayCustomVo != null) {
					if (al_PayCustomVo.size() > 0) {
						...
						for (int i = 0; i < al_PayCustomVo.size(); i++) {
							...
							logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "pay_bill_seq : "
									+ pay_bill_seq + "\n");
							logData.append(
									"[" + LogUtils.getCurrentTime() + "]" + " " + "access_seq : " + access_seq + "\n");
							logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "pay_type_code : "
									+ pay_type_code + "\n");
							logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "id : " + id + "\n");
							logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "affiliates_code : "
									+ affiliates_code + "\n");
							logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "url : " + url + "\n");
							logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "price : " + price + "\n");
							logData.append(
									"[" + LogUtils.getCurrentTime() + "]" + " " + "currency : " + currency + "\n");
							logData.append(
									"[" + LogUtils.getCurrentTime() + "]" + " " + "goodname : " + goodname + "\n");
							logData.append(
									"[" + LogUtils.getCurrentTime() + "]" + " " + "buyername : " + buyername + "\n");
							logData.append(
									"[" + LogUtils.getCurrentTime() + "]" + " " + "buyertel : " + buyertel + "\n");
							logData.append(
									"[" + LogUtils.getCurrentTime() + "]" + " " + "buyeremail : " + buyeremail + "\n");
							logData.append(
									"[" + LogUtils.getCurrentTime() + "]" + " " + "cardquota : " + cardquota + "\n");
							logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "billkey : " + billkey + "\n");
							logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "pay_success_seq : "
									+ pay_success_seq + "\n");
							...
							if (!"G-PAY".equals(p_type)) {
								// 이니시스 정기결제 승인
								data = payController.iniCardBill(price, goodname, buyername, buyertel, buyeremail,
										billkey, logData);
								...
								if ("00".equals(S_RESULTCODE)) {
									...
									logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "S_RESULTCODE:"
											+ S_RESULTCODE + "\n");
									logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "S_RESULTMSG : "
											+ S_RESULTMSG + "\n");
									...
									logData.append(
											"[" + LogUtils.getCurrentTime() + "]" + " " + "S_TID:" + S_TID + "\n");
									logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "S_AUTHCODE : "
											+ S_AUTHCODE + "\n");
									logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "S_PGAUTHDATE : "
											+ S_PGAUTHDATE + "\n");
									logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "S_PGAUTHTIME : "
											+ S_PGAUTHTIME + "\n");
									...
									try {
										...
										e.printStackTrace();
										logData.append(
												"[" + LogUtils.getCurrentTime() + "]" + " " + e.toString() + "\n");
								...
								if (subscriptionPurchase != null && subscriptionPurchase.getAutoRenewing()) {
									...
									logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "subscriptionPurchase:"
							...
							if ("00".equals(S_RESULTCODE) && !"".equals(StringUtil.nvl(S_P_TID))
									&& !"".equals(StringUtil.nvl(S_P_AUTH_DT))) {
								...
								try {
									logData.append(
											"[" + LogUtils.getCurrentTime() + "]" + " " + "insertPayBillBatch" + "\n");
									success = payService.insertPayBillBatch(payCustomVo, logData);
									logData.append(
											"[" + LogUtils.getCurrentTime() + "]" + " " + "success:" + success + "\n");
									if (success > 0) {
										returnValue = 1;
										log_msg += "DB 등록 성공";
									} else {
									...
									logData.append("[" + LogUtils.getCurrentTime() + "]" + " "
											+ "insertPayBillBatch error e:" + e.toString() + "\n");
									returnValue = 0;
							...
							if (1 != returnValue) {
								...
								if (!"G-PAY".equals(payCustomVo.getP_type())) {
									...
									try {
										...
										if (inipay != null) {
											log_msg += inipay.GetResult("ResultCode") + " "
							...
							if (!"00".equals(S_RESULTCODE)) {
								...
								try {
									...
									logData.append("[" + LogUtils.getCurrentTime() + "]" + " "
											+ "insertPayBillFail PayCustomVo payCustomVo : " + payCustomVo + "\n");
									...
									e.printStackTrace();
									logData.append("[" + LogUtils.getCurrentTime() + "]" + " "
											+ "insertPayBillFail error e:" + e.toString() + "\n");
								}
								logData.append("[" + LogUtils.getCurrentTime() + "]" + " "
										+ "insertPayBillFail success:" + success + "\n");
								returnValue = 0;
							}

							if (returnValue >= 1) {
								logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "결제 성공" + "\n");
								logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "payCustomVo : "
										+ payCustomVo + "\n");
							} else {
							...
							try {
								...
								logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "inserttPayExeLog success:"
										+ success + "\n");
							} catch (Exception e) {
								e.printStackTrace();
								logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "inserttPayExeLog error e:"
										+ e.toString() + "\n");
							}
			...
			try {
				...
				e.printStackTrace();
				logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "query error sendMailForm : "
						+ e.toString() + "\n");
			}
			logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "query end sendMailForm int success : "
					+ success + "\n");

			long endTime = System.currentTimeMillis() - strartTime;
			logData.append(
					"[" + LogUtils.getCurrentTime() + "]" + " " + "tag:" + tag + ":전체:endTime:" + endTime + "\n");
			if (endTime > 15000) {
				logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "tag:" + tag + ":전체:endTime:" + endTime
						+ ",connection time out[15second over]" + "\n");
			}
				...
				if (al_PayCustomVo != null) {
					if (al_PayCustomVo.size() > 0) {
						for (int i = 0; i < al_PayCustomVo.size(); i++) {
							...
							// 구글구독 상태 확인
							SubscriptionPurchase subscriptionPurchase = GoogleInApp
									.checkSubscriptionPurchase(subscriptionId, token);
							logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "subscriptionPurchase:"
									+ subscriptionPurchase + "\n");
							if (subscriptionPurchase != null) {
								...
								if (auto_renewing) {
									...
									if (payment_satate == 1) {
										...
										if (compare > 0) {
											...
											if (!o_id.equals(subscriptionPurchase.getOrderId())) {
												// 새로운 오더아이디 일경우
												logData.append("[" + LogUtils.getCurrentTime() + "]" + " "
														+ "결제 성공(만료일이 기존 만료일 보다 큰경 된 경우) 새로운 오더아이디 일경우" + "\n");
												int success = 0;
												try {
													String p_tid = subscriptionPurchase.getOrderId();
													p_auth_dt = expiryDate;
													Date batch_date = expiryDate;
													int cancelReason = (subscriptionPurchase.getCancelReason() == null)
															? 0
															: subscriptionPurchase.getCancelReason();
													payCustomVo.setP_tid(p_tid);
													payCustomVo.setP_auth_dt(p_auth_dt);

													Calendar cal = Calendar.getInstance();

													SimpleDateFormat db_dt = new SimpleDateFormat(
															"yyyy-MM-dd HH:mm:ss");
													...
													if (compare > 0) {
														...
														cal.add(Calendar.DATE, -1);
														payCustomVo.setBatch_date(new Date(cal.getTimeInMillis()));// 티켓
																													// 만료일
																													// -1일
													} else {
														...
														cal.add(Calendar.DATE, -1);
														payCustomVo.setBatch_date(new Date(cal.getTimeInMillis()));// 구독
																													// 만료일
																													// -1일
													}
													payCustomVo.setPay_bill_seq(pay_bill_seq);
													logData.append("[" + LogUtils.getCurrentTime() + "]" + " "
															+ "updateGoogleBillPayBillBatch PayCustomVo payCustomVo : "
															+ payCustomVo + "\n");
													success = payService.updateGoogleBillPayBillBatch(payCustomVo);
												} catch (Exception e) {
													e.printStackTrace();
													logData.append("[" + LogUtils.getCurrentTime() + "]" + " "
															+ "updateGoogleBillPayBillBatch error e:" + e.toString()
															+ "\n");
												}
												logData.append("[" + LogUtils.getCurrentTime() + "]" + " "
														+ "updateGoogleBillPayBillBatch success:" + success + "\n");
											} else {
												// 기존 오더아이디 일경우
												logData.append("[" + LogUtils.getCurrentTime() + "]" + " "
														+ "결제 보류(만료일이 기존 만료일 보다 큰 경우) 기존 오더아이디 일경우" + "\n");
											}
										} else {
									...
									logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "결제 실패 auto_renewing:"
											+ auto_renewing + "\n");
									String log_cancel_msg = "";
								...
								// 구독 상태 확인 실패
								logData.append("[" + LogUtils.getCurrentTime() + "]" + " "
										+ "subscriptionPurchase is null(구독 상태 확인 실패) payCustomVo:" + payCustomVo
										+ "\n");
							}
						}
					} else {
	...
	public int insertUserRefund(String access_seq, String pay_success_seq, String pay_bill_seq, StringBuffer logData) {
		...
		try {
			...
			logData.append("[" + LogUtils.getCurrentTime() + "]" + " "
					+ "query start insertUserRefund TbUserRefund tbUserRefund:" + tbUserRefund + "\n");
			success = payService.insertUserRefund(tbUserRefund);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "query error insertUserRefund : "
					+ e.toString() + "\n");
		}
		logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "query end insertUserRefund int success : "
				+ success + "\n");
		return success;
	}
