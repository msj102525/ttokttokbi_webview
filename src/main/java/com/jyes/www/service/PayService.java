...
public class PayService implements IPayService {
	...
	public int insertPayInfo(PayCustomVo payCustomVo, StringBuffer logData) throws SQLException {
		...
		if(!"".equals(StringUtil.nvl(payCustomVo.getBillkey()))||
				("G-PAY".equals(StringUtil.nvl(payCustomVo.getP_type()))&&"1".equals(StringUtil.nvl(payCustomVo.getCheckPurchaseType())))) {
			returnValue = 0;
		...
		try {
			if(payCustomVo_sub != null) {
				if(0 < Integer.parseInt(payCustomVo_sub.getUse_day())) {
					...
					//쿠폰 사용자중 최초 유료 결제 통계
					try {
						logData.append("["+LogUtils.getCurrentTime()+"]"+" "+"selectStatsDayCuponUserCheck payCustomVo:"+payCustomVo+"\n");
						//쿠폰 사용자의 첫번째 유료 결제시 마지막 사용한 쿠폰 번호 가져오기
						String last_coupon_num = (String)payMapper.selectStatsDayCuponUserCheck(payCustomVo);
						logData.append("["+LogUtils.getCurrentTime()+"]"+" "+"selectStatsDayCuponUserCheck last_coupon_num:"+last_coupon_num+"\n");
						payCustomVo.setCoupon_num(last_coupon_num);
						if(!"".equals(StringUtil.nvl(last_coupon_num))) {
							...
							payCustomVo.setStore_type(accessVo.getStore_type());
							logData.append("["+LogUtils.getCurrentTime()+"]"+" "+"insertStatsDayCupon payCustomVo:"+payCustomVo+"\n");
							payMapper.insertStatsDayCupon(payCustomVo);
						}
					} catch(Exception e) {
						e.printStackTrace();
					}
				}
				if(0 < Integer.parseInt(payCustomVo_sub.getMake_count())) {
					int length = Integer.parseInt(payCustomVo_sub.getMake_count());
					for (int i = 0; i < length; i++) {
						returnValue = 0;
						returnValue = payMapper.insertMakeProductionRequest(payCustomVo);
						if (returnValue < 1) {
							throw new SQLException("i:"+i+":insertMakeProductionRequest 결과 없음");
						}
					}
				}
			} else {
...
//		hp_no = hp_no1+"-"+hp_no2+"-"+hp_no3;
		 ...
		 * 스마트로 이용권 정보 DB 저장
		 */
		if("A0001".equals(accessVo.getAffiliates_code())&&!"G-PAY".equals(payCustomVo.getP_type())) {
			/**
			 * 비즐 아이디 고유 번호 조회
			 */
			try {
				...
				if(response!=null&&response.getResponse()!=null) {
					CheckMemberVo checkMemberVo = com.jyes.www.network.JsonParser.parseCheckMemberVo(response.getResponse().toString());
					logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "CheckMemberVo checkMemberVo : " + checkMemberVo + "\n");
			...
			smartroPurchaseInfoVo.setPayment("C");//C: 신용카드 (Credit Card), P: 휴대폰 결제 (Phone), A: 계좌이체 (Account), V: 이용권(Voucher), O: 기타(Other)
...
//				json_params.put("grant_type", OneStore.GRANT_TYPE);변수 초기화가 암됨 ㅠㅠ
				...
				if(response!=null) {
					...
					logData.append("["+LogUtils.getCurrentTime()+"]"+" "+"json_response:"+json_response+"\n");
					if(json_response.get("error")==null) {
						access_token = json_response.get("access_token").getAsString();
						logData.append("["+LogUtils.getCurrentTime()+"]"+" "+"access_token:"+access_token+"\n");
					} else {
					...
					if(response!=null) {
						...
						if(json_response.get("error")==null) {
							...
							if("0".equals(responseCode)) {
								...
								logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "query start updateStorePurchaseInfo StorePurchaseInfoVo storePurchaseInfoVo : " + storePurchaseInfoVo + "\n");
								resultCnt = payMapper.updateStorePurchaseInfo(storePurchaseInfoVo);
								logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "query end updateStorePurchaseInfo int resultCnt : " + resultCnt + "\n");
							}
						} else {
							String code = json_response.get("error").getAsJsonObject().get("code").getAsString();
							logData.append("["+LogUtils.getCurrentTime()+"]"+" "+"code:"+code+"\n");
...
//				req.addParameter("cancelId", smartroPurchaseInfoVo.getO_id());//취소시
				...
				req.addParameter("payMethod", "C");//C: 신용카드 (Credit Card), P: 휴대폰 결제 (Phone), A: 계좌이체 (Account), V: 이용권(Voucher), O: 기타(Other)
				...
				logData.append("["+LogUtils.getCurrentTime()+"]"+" "+"response:"+response+"\n");
				if(response!=null&&response.getResponse()!=null) {
					PayInfoRequestVo payInfoRequestVo = com.jyes.www.network.JsonParser.parsePayInfoRequestVo(response.getResponse().toString());
					...
					if("SUCCESS".equals(result)) {
						...
						logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "query start updateSmartroPurchaseInfo StorePurchaseInfoVo storePurchaseInfoVo : " + storePurchaseInfoVo + "\n");
						resultCnt = payMapper.updateSmartroPurchaseInfo(smartroPurchaseInfoVo);
						logData.append("[" + LogUtils.getCurrentTime() + "]" + " " + "query end updateSmartroPurchaseInfo int resultCnt : " + resultCnt + "\n");	
					}
				}
			} catch(Exception e) {
				e.printStackTrace();
...
//		if("".equals(StringUtil.nvl(tbUserRefund.getSeq()))) {
			...
			tbUserTicketsDateCheck = (TbUserTicketsDateCheck)payMapper.getUserTicketsDateCheck(tbUserTicketsDateCheck_request);
				...
				try {
					...
					if(compare_start < 0) {
						...
						if(Integer.parseInt(StringUtil.nvl(tbUserTicketsDateCheck.getUse_day(),"0")) > 0) {
							...
							if(returnValue < 1) {
//								throw new SQLException("updateUserTicketsDate 결과 없음");//제일 마지막 이용권일 경우 없을 수 있음
								returnValue = 1;
							}
						} else {
							//현재 시간이 크거나 같다
